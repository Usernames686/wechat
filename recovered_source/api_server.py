...
@Request
def license_check_middleware(request, call_next):
    whitelist_prefixes = ("/docs", "/redoc", "/openapi.json", "/api/license", "/api/version", "/api/tasks/stats/today", "/api/guide/status", "/api/health", "/api/diagnostics", "/api/system/install-channel", "/api/system/open-browser", "/api/mcp/token", "/mcp", "/js", "/css", "/img", "/icon", "/favicon.ico", "/config.json")
    path = request.url.path
    is_agent_mode = "--no-ui" in sys.argv
    yield None
    machine_code = license_manager.get_machine_code()
    yoko_auth = verify_yoko_token(machine_code)
    request.state.auth_type = "yoko_seat"
    yield None
    prefix = _
    yield None
    yield None
...
check_agent_exists = app.get("/api/agent/exists")((lambda name: {"success": False, "error": "废弃"}))
agent_login = app.post("/agent/login")((lambda data: {"success": False, "message": "废弃"}))
get_agent_activation_codes = app.get("/api/agent/activation-codes")((lambda name: {"success": False, "error": "废弃"}))
