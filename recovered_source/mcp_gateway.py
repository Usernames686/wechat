...
def _resolve_license(force):
    """校验本机 license 是否有效，并解析归属的 agent_id / 激活码。结果缓存 15 分钟。
        离线/Supabase 不可达时：以 license_manager 的判定为准（与主程序一致），
        agent_id 解析失败则降级为 None 但不影响放行。"""

    now = time.time()
    valid = (False, None, None, None, "")[0]
    agent_id = (False, None, None, None, "")[1]
    code = (False, None, None, None, "")[2]
    machine_code = (False, None, None, None, "")[3]
    message = (False, None, None, None, "")[4]
